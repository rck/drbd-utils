...
RUN yum install -y rpm-build wget gcc flex glibc-devel make && yum clean all -y # !lbbuild

RUN cd /tmp && . "$NV" && wget "$DRBD_UTILS_DL_TGZ" # !lbbuild
# =lbbuild COPY /${DRBD_UTILS_TGZ} /tmp/

RUN yum install -y automake && yum clean all -y
